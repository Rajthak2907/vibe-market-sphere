...
const Index = () => {
  ...
  const SectionHeader = ({
    ...
    gradient?: string;
      ...
      {viewAllLink && <Link to={viewAllLink} className="text-sm text-obeyyo-blue hover:text-obeyyo-pink flex items-center gap-1 font-medium">
  ...
  const BankOffersSection = () => {
    const sliderRef = useRef<HTMLDivElement>(null);
    return <section className="bg-gradient-to-r from-obeyyo-blue to-obeyyo-pink mx-4 rounded-2xl p-4 bg-zinc-950">
        ...
        <div ref={sliderRef} className="flex overflow-x-auto space-x-4 pb-2 scrollbar-hide scroll-smooth">
          {bankOffers.map(offer => <div key={offer.id} className="flex-shrink-0 w-72">
      ...
      <div className="space-y-6 bg-gray-50">
        ...
        <div className="px-4">
          <div className="grid grid-cols-4 gap-3">
            ...
            link: "/budget",
            color: "#fb8619"
          }].map(action => <Link key={action.label} to={action.link} className="bg-white rounded-2xl p-4 text-center shadow-sm hover:shadow-md transition-all" style={{
        ...
        <section className="bg-gradient-to-r from-obeyyo-orange to-obeyyo-yellow mx-4 rounded-2xl p-4 bg-white">
          ...
          {isLoading ? <div className="flex gap-4 overflow-x-auto">
              {Array.from({
            length: 4
          }).map((_, i) => <div key={i} className="flex-shrink-0 w-32">
                  <SkeletonLoader type="brand" />
                </div>)}
            </div> : <FeaturedBrands brands={featuredBrands} />}
        </section>
